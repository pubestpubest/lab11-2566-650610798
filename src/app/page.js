...
export default function RegisFormPage() {
  ...
  const [gnederError, setGenderError] = useState(false);
  ...
  function computeTotalPayment() {
    ...
    if (buyCap) total += 400;
    let buyAll = buyBottle && buyCap && buyShoes;
    if (buyAll) total *= 0.8;
    return total;
  }
  ...
  return (
    <div className="mx-auto vstack gap-3" style={{ width: "400px" }}>
      ...
      <div>
        <input className="me-2" type="checkbox" onChange={termOnChange} />I
        agree to the terms and conditions
      </div>

      {/* Register Button */}
      <button
        className={"btn btn-success my-2" + (term ? "" : " disabled")}
